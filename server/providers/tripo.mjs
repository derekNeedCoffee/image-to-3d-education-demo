...
export async function createTripoTask(payload) {
  ...
  const fileName = sanitizeFileName(payload.fileName || `animal-reference.${image.ext}`);
...
export async function getTripoTask(taskId) {
  ...
  if (rawModelUrl && isSuccessStatus(status)) {
    try {
      ...
      cacheError = error.message || '模型缓存失败。';
    }
  }
...
function requireTripoKey() {
  if (!TRIPO_API_KEY) {
    const error = new Error('后端没有配置 TRIPO_API_KEY。请复制 .env.example 为 .env.local 并填入 Tripo key。');
    error.status = 500;
    throw error;
  }
}
...
async function uploadImageToTripo({ buffer, mime, fileName }) {
  ...
  if (!host || !bucket || !key || !tokenData.sts_ak || !tokenData.sts_sk || !tokenData.session_token) {
    const error = new Error('Tripo STS 上传 token 缺少必要字段。');
...
async function uploadToTripoObjectStorage({ buffer, mime, host, bucket, key, accessKeyId, secretAccessKey, sessionToken }) {
  ...
  const fetchOptions = OUTBOUND_PROXY_AGENT ? { dispatcher: OUTBOUND_PROXY_AGENT } : {};
  ...
  if (!response.ok) {
    ...
    const error = new Error(`Tripo 对象存储上传失败，HTTP ${response.status}。`);
...
function normalizeTaskCreateResponse(raw) {
  ...
  if (!taskId) {
    const error = new Error('Tripo 创建任务响应里没有 task id。');
    error.detail = raw;
    throw error;
  }
...
async function tripoRequest(requestPath, options = {}) {
  ...
  try {
    ...
    const wrapped = new Error(`Tripo 网络请求失败：${error.message}`);
    wrapped.detail = {
      path: requestPath,
  ...
  try {
    ...
    data = { message: text || 'Tripo 返回了非 JSON 响应。' };
  }

  if (!response.ok || (typeof data.code === 'number' && data.code !== 0)) {
    const error = new Error(data.message || data.error || `Tripo 请求失败，HTTP ${response.status}。`);
    error.status = response.status || 502;
    error.detail = sanitizeTripoRaw(data);
    throw error;
  }
